...
// Descripción del modulo
    ...
    .setName('setmedalla')
    .setDescription('Le setea medallas a alguien.')
    ...
    async execute(interaction) {
        ...
        // Log
        console.log(`${interaction.user.tag} en el canal de #${interaction.channel.name} triggereó la interación de setear medallas. `);
        ...
        .setDescription(`Le estás seteando medallas a <@${newuserid.id}>: ${medallaoro} <:oro:904439890777874452> ${medallaplata} <:plata:904439889175642142> ${medallabronce} <:bronce:904439889867702324> ${medallahierro} <:hierro:904439890417180683> ${medallamadera} <:carton:904439891037929522>`);
        ...
        collector.on('collect', i => {
            collector.stop();
            i.deferUpdate();
            var new_score = (medallaoro * 5) + (medallaplata * 4) + (medallabronce * 3) + (medallahierro * 2) + (medallamadera * 1);
            var new_gold = medallaoro;
            var new_silver = medallaplata;
            var new_bronze = medallabronce;
            var new_iron = medallahierro;
            var new_cardboard = medallamadera;
